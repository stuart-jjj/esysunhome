...
to a `configId` mismatch, not a missing/renamed register.** Symptom was broader than the grid-power
issue above: SOC, load power, grid power, grid voltage, grid frequency, and total generation were
all silently wrong (not missing/zero in the usual way — `gridVoltage` read 0.8V, `gridFrequency`
